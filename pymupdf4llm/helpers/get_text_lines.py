...
def get_raw_lines(textpage, clip=None, tolerance=3):
    ...
    blocks = [b for b in textpage.extractDICT()["blocks"] if b["type"] == 0 and not pymupdf.Rect(b["bbox"]).is_empty]
    ...
    for bno, b in enumerate(blocks):
        for lno, l in enumerate(b["lines"]):
            for s in l["spans"]:
                ...
                if abs(sbbox & clip) < abs(sbbox) * 0.8:  # must be inside parameter rectangle
                    continue
