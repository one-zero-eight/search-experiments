...
from pymupdf4llm.rectangle_utils import is_in_rects, any_rect_between_two_rects
...
def column_boxes(
    ...
    *,
    ...
    # path rectangles
    path_bboxes = [p["rect"] for p in paths]
    # sort path bboxes by ascending top, then left coordinates
    path_bboxes.sort(key=lambda b: (b.y0, b.x0))

    # bboxes of images on page, no need to sort them
    for item in page.get_images():
        avoid_bboxes.extend(page.get_image_rects(item[0]))

    # blocks of text on page
    text_blocks = textpage.extractDICT()["blocks"]
    text_blocks = list(filter(lambda b: not is_in_rects(b["bbox"], avoid_bboxes), text_blocks))
    bboxes = []
    ...
    bboxes.sort(key=lambda k: (is_in_rects(k, path_bboxes), k.y0, k.x0))
    ...
    for i, bb in enumerate(bboxes):  # iterate old bboxes
        ...
        for j in range(len(nblocks)):
            ...
            if is_in_rects(nbb, path_bboxes) != is_in_rects(bb, path_bboxes):
                continue

            # never join across figures
            if any_rect_between_two_rects(nbb, bb, avoid_bboxes):
                continue
