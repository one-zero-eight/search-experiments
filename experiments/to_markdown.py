...
from typing import overload, Literal, TypedDict, Callable
...
class IdentifyHeaders:
    ...
    def __init__(
        ...
        # identify up to 2 font sizes as header candidates
        sizes = sorted(
    ...
    def get_header_id(self, span: dict, page=None) -> int | None:
        """Return appropriate markdown header prefix.

        Given a text span from a "dict"/"rawdict" extraction, determine the
        markdown header level (0 - maximal).
        """
...
def rindex_for_list(alist, value):
...
def to_markdown(
    ...
    *,
    ...
    def save_image(page: fitz.Page, rect, i):
        ...
        pix: fitz.Pixmap = fitz.utils.get_pixmap(page, clip=rect)
        if callable(write_images) and not write_images(pix):
            del pix
            return None
        pix.save(image_path)
        del pix
        return f"{page.number}-{i}.png", os.path.basename(image_path)
    ...
    def write_text(
        ...
        for lrect, spans in nlines:
            ...
            hdr_level = get_header_id(span0, page=page)

            if hdr_level is not None:
                stack.append(f"<h{hdr_level}>")
            else:
                if prev_hdr_level:
                    stack.append(f"</h{prev_hdr_level}>")

            prev_hdr_level = hdr_level
            prev_link = None
            prev_mono = False
            prev_bold = False
            prev_italic = False
            ...
            for i, s in enumerate(spans):  # iterate spans of the line
                ...
                bold = s["flags"] & 16
                italic = s["flags"] & 2
                link = resolve_links(links, s)

                if link and not prev_link:
                    stack.append(f'<a href="{link}">')
                if mono and not prev_mono:
                    stack.append("<code>")
                if bold and not prev_bold:
                    stack.append("<b>")
                if italic and not prev_italic:
                    stack.append("<i>")

                _closing = []

                if prev_link and not link:
                    _closing.append((f'<a href="{prev_link}">', "</a>"))
                if prev_mono and not mono:
                    _closing.append(("<code>", "</code>"))
                if prev_bold and not bold:
                    _closing.append(("<b>", "</b>"))
                if prev_italic and not italic:
                    _closing.append(("<i>", "</i>"))

                # sort by distance to the last opening tag
                _closing.sort(
                    key=lambda x: rindex_for_list(stack, x[0]),
                    reverse=True,
                )
                stack.extend([x[1] for x in _closing])
            ...
            if prev_italic:
                _closing.append(("<i>", "</i>"))
            if prev_hdr_level:
                _closing.append((f"<h{prev_hdr_level}>", f"</h{prev_hdr_level}>"))
    ...
    def get_page_output(doc, pno, margins, textflags):
        ...
        for text_rect in text_rects:
            sorted_layout.append({"type": "text", "rect": text_rect})

        for key, tab_rect in tab_rects.items():
            sorted_layout.append({"type": "tab", "rect": tab_rect, "key": key})

        for key, img_rect in vg_clusters.items():
            sorted_layout.append({"type": "image", "rect": img_rect, "key": key})
        ...
        for obj in sorted_layout:
            if obj["type"] == "text":
                ...
                tab = tabs[obj["key"]]
                md_string += "\n" + tab.to_markdown(clean=False)
            elif obj["type"] == "image":
                displayname, pathname = save_image(page, obj["rect"], obj["key"])
                if pathname:
                    md_string += GRAPHICS_TEXT % (displayname, quote(pathname))
